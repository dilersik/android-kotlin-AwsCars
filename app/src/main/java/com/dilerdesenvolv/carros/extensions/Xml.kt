...
class Xml(val root: Element) {
    ...
    // Retorna todos elementos filhos desta tag
...
// Retorna objeto XML a partir da String
fun String.getXML(charset: String = "UTF-8"): Xml {
    try {
        val xml = this
        val factory = DocumentBuilderFactory.newInstance()
        factory.isValidating = false
        val inputStream = ByteArrayInputStream(xml.toByteArray(kotlin.text.charset(charset)))

        return Xml(factory.newDocumentBuilder().parse(inputStream).documentElement
                ?: throw RuntimeException("XML incorreto."))
